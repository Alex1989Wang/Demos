...
{
    __weak typeof(self) weakSelf = self;
    NSInteger photoCount = [[PhotoManager sharedManager].photos count];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(),
                   ^{
                       __strong typeof(weakSelf) strSelf = weakSelf;
                       NSString *prompt = (!photoCount) ?
                       @"Add photos with faces to googlyify them." : nil;
                       [strSelf.navigationItem setPrompt:prompt];
                   });
}
