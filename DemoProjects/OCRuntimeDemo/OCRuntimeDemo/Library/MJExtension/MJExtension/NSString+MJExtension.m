...
- (NSString *)mj_underlineFromCamel
{
    ...
    BOOL isLastCharacterUpper = NO;
    BOOL isPresentCharacterUpper = NO;
    for (NSUInteger i = 0; i<self.length; i++) {
        unichar c = [self characterAtIndex:i];
        NSString *cString = [NSString stringWithFormat:@"%c", c];
        NSString *cStringLower = [cString lowercaseString];
        isPresentCharacterUpper = ![cString isEqualToString:cStringLower];
        
        if (!isLastCharacterUpper && isPresentCharacterUpper && i) {
        ...
        isLastCharacterUpper = isPresentCharacterUpper;
    }
    return string;
}
