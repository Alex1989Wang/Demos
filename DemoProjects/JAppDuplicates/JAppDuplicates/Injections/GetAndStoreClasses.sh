...
# 属性黑名单配置文件
blacklist_cfg_file="$(pwd)/configures/DefaultClassesBlackListConfig.cfg"
...
post_implement_file_handle() {
	...
	# 添加最后一个类
	if [[ ${#lastLine} -le 6 ]] || [[ $(expr "$lastLine" : '^#.*') -gt 0 ]] || [[ -n ${grep_result} ]] || [[ ${lastLine} =~ ${category_judge_substring} ]]; then
		# 长度小于等于6、注释内容的行、在黑名单中的内容、分类文件不处理
		echo "less then 6 char line or comment line ==> ${line}"
	else
		if [[ -n ${lastLine} ]]; then
			echo ${lastLine} >> ${wirte_to_file}
		fi
