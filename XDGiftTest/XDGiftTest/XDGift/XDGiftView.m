...
- (void)setUpFirst
{
    ...
    for (NSInteger i = 0; i < 3; i++) {
        ...
        XDGigtUserView *userV = [[XDGigtUserView alloc] init];
...
#warning - 把点击的view对应的用户信息保存起来，知道给谁送礼物
    XDGigtUserView *v = (XDGigtUserView *)tap.view;
    v.big = NO;
    ...
    for (UIView *subV in self.topView.subviews) {
        ...
        if ([subV isKindOfClass:[XDGigtUserView class]] && subV != tap.view) {
            ...
            XDGigtUserView *vv = (XDGigtUserView *)subV;
            vv.big = YES;
        }
    }
...
- (void)layoutviews:(CGFloat)scale
{
    ...
    for (NSInteger i = 0; i < self.topView.subviews.count; i++) {
        ...
        XDGigtUserView *subV = self.topView.subviews[i];
        ...
        if (i == 2) {
            XDGigtUserView *subV1 = self.topView.subviews[i - 1];
...
#warning - 时间判断 要求5秒内连击
    // 1 创建礼物视图
//    self.aniGiftView.count = number;
    self.aniGiftView.giftImageView.image = self.seleCell.image;
...
-(XDAniGiftView *)aniGiftView
{
    if (_aniGiftView == nil) {
        ...
        XDAniGiftView *aniGift = [XDAniGiftView aniGiftView];
        aniGift.acceptType = 3;                                      // 模拟类型************
